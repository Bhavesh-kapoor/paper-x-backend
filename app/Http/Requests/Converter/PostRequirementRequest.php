...
class PostRequirementRequest extends ApiRequest
{
    ...
    {
        return [
            // Required: Inquiry Type & Intent (material with buy or sell intent for converter requirements)
            ...
            'visibility' => [
                'required',
                'string',
                Rule::in(['dealers', 'converters', 'all'])
            ],
            
            // Required: Location (converters don't have saved locations, so only manual)
            'location_source' => [
                'required',
                'string',
                Rule::in(['manual']) // Converters only use manual location
            ],
            'location' => [
                'required',
                'string',
            ...
            // Optional: Location ID (should be null for converters)
            'location_id' => [
                'nullable',
                'integer'
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'urgency.in' => 'Urgency must be "normal" or "urgent"',
    ...
    /**
     * Custom validation: Ensure location_id is null for converters (they don't have saved locations)
     */
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // Converters only use manual location, so location_id should be null
