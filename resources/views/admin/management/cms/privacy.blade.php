...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="page-title">
            <i class="icon-base ti tabler-shield-lock me-2"></i>Privacy Policy
        </h4>
    ...
    <div class="card privacy-card">
        ...
        <div class="card-body">
            <form id="privacyForm" method="POST" action="{{ route('admin.cms.privacy.store') }}">
                @csrf
                <div class="mb-3">
                    <label class="form-label">Content <span class="text-danger">*</span></label>
                    <textarea name="content" id="content" class="form-control" rows="20" required style="color: #212529; background: #ffffff; display: none;">{{ $privacy->content ?? '' }}</textarea>
                ...
                <div class="d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn-save" id="saveBtn">
                        ...
                        <span id="saveBtnText">Save Privacy Policy</span>
                    </button>
                </div>
            </form>
        </div>
...
document.getElementById('privacyForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success !== false) {
            showAlert('Privacy Policy updated successfully!', 'success');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showAlert(error.message || 'Error saving Privacy Policy', 'danger');
    })
    .finally(() => {
        submitBtn.disabled = false;
        spinner.classList.add('d-none');
        btnText.textContent = 'Save Privacy Policy';
    });
});
